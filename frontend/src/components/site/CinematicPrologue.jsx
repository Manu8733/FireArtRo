...
export const CinematicPrologue = () => {
  ...
  return (
    ...
    >
      <motion.div
        className="sticky top-0 h-[100svh] overflow-hidden bg-[#050308]"
        style={reduce ? undefined : { scale: frameScale, clipPath: frameClip }}
      >
        <AnimatePresence initial={false}>
          <motion.div
            ...
            animate={{
              ...
              y: 0,
              clipPath: scene.motion === "burst" ? "circle(100% at 50% 50%)" : "inset(0% 0% 0% 0% round 0px)",
            }}
        ...
        <motion.div
          className="absolute -right-[20vw] top-1/2 h-[62vw] w-[62vw] -translate-y-1/2 rounded-full blur-[120px]"
        ...
        <div className="relative z-10 mx-auto flex h-full max-w-7xl items-end px-5 pb-24 sm:px-6 md:items-center md:px-12 md:pb-0">
          <AnimatePresence mode="wait">
            <motion.div
              ...
              exit={reduce ? undefined : { opacity: 0, y: -26, filter: "blur(8px)" }}
              transition={{ duration: reduce ? 0 : 0.6, ease: EASE }}
              className="max-w-2xl"
            >
              <span className="cine-kicker text-[10px] font-semibold text-[#9D7BFF]">{scene.kicker}</span>
        ...
        <div className="absolute bottom-7 left-1/2 z-20 flex -translate-x-1/2 items-center gap-2">
          {SCENES.map((item, index) => (
            ...
            >
              <span className={`block h-1.5 rounded-full transition-all duration-500 ${index === active ? "w-12 bg-gradient-to-r from-[#3A86FF] to-[#8338EC]" : "w-2 bg-white/22"}`} />
            </button>
          ))}
        </div>
      </motion.div>
