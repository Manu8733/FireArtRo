    ...
    <div className="mx-auto grid max-w-7xl gap-10 px-5 sm:px-6 md:px-12 lg:grid-cols-[0.82fr_1.18fr] lg:gap-16">
      <Reveal>
        <div className="lg:sticky lg:top-28">
          <span className="cine-kicker text-[10px] font-semibold text-[#9D7BFF]">Control înainte de impact</span>
          ...
          <div className="relative mt-10 hidden aspect-square max-w-[320px] items-center justify-center lg:flex">
            ...
            <div className="absolute inset-[18%] rounded-full border border-dashed border-[#8338EC]/25 animate-[spin_28s_linear_infinite]" />
            <div className="absolute inset-[31%] rounded-full bg-[#8338EC]/12 blur-2xl" />
            <span className="relative flex h-24 w-24 items-center justify-center rounded-full border border-white/12 bg-[#0A0712] shadow-[0_0_60px_rgba(131,56,236,0.25)]">
              <ShieldCheck className="h-10 w-10 text-[#9D7BFF]" />
            </span>
          </div>
        </div>
      </Reveal>
          ...
          <Reveal key={item.title} delay={(index % 3) * 0.05}>
            <article className="group grid grid-cols-[auto_1fr] gap-4 border-b border-white/10 py-5 sm:grid-cols-[54px_0.75fr_1.25fr] sm:items-center sm:gap-6 sm:py-6">
              ...
              <div className="flex items-center gap-3">
                <span className="flex h-9 w-9 shrink-0 items-center justify-center rounded-xl border border-white/9 bg-white/[0.035] transition-colors group-hover:border-[#8338EC]/35">
                  <item.icon className="h-4 w-4 text-[#9D7BFF]" />
                </span>
